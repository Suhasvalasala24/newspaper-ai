...
(function () {
  ...
  async function submitMessage(el, config) {
    ...
    try {
      ...
      const displayErr = currentLang === 'te'
        ? `❌ లోపం: ${errText}`
        : `❌ Error: ${errText}`;
      appendMessage(el.messages, 'bot', displayErr);
    } finally {
      isTyping      = false;
      el.send.disabled = false;
    }
  ...
  const TOPIC_FILTERS = [
    ...
    { triggers: ['technology','cyber','tech','ai','mobile','app','software','it sector','సాంకేతిక','సైబర్','టెక్','మొబైల్','యాప్'],
      section: 'Technology',
  ...
  function buildTopicContext(section, matching) {
    ...
    matching.forEach(a => {
      ...
      if (body.length >= 150 && body !== a.headline) {
        ...
        // The LLM must copy the Summary field verbatim — no paraphrase, no additions.
...
⛔ HALLUCINATION FORBIDDEN: Do not generate descriptions. Do not rephrase. Print the summary text content verbatim, never the "Summary:" label.
...
⛔ NEVER print "[HEADLINE ONLY...]" — it is an internal marker, invisible to readers.

⛔ HALLUCINATION IS FORBIDDEN: Do not write any sentence that does not appear word-for-word in TODAY'S ARTICLES. Print summary text content verbatim, NEVER the "Summary:" label.
